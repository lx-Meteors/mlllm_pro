...
def LlamaForCausalLM__lora_forward(
    ...
    if self.config.pretraining_tp > 1:
        ...
        logits = self.lm_head(hidden_states[:, -num_logits_to_keep:, :], enabled_lora)
...
def LlamaModel_lora_forward(
    ...
    if inputs_embeds is None:
        inputs_embeds = self.embed_tokens(input_ids, enabled_lora)
...
def LlamaDecoderLayer_lora_forward(
    ...
    **kwargs,
    ...
    hidden_states = self.mlp(hidden_states, enabled_lora)
    hidden_states = residual + hidden_states
...
def LlamaSdpaAttention_lora_forward(
    ...
    **kwargs,
    ...
    query_states = self.q_proj(hidden_states, enabled_lora)
    key_states = self.k_proj(hidden_states, enabled_lora)
    value_states = self.v_proj(hidden_states, enabled_lora)
    ...
    attn_output = self.o_proj(attn_output, enabled_lora)

    return attn_output, None, past_key_value


def LlamaMLP_lora_forward(self, x, enabled_lora): # x[batch_size, seq_len, hidden_size]
    if self.config.pretraining_tp > 1: # 假如pretraining_tp=2 张量并行切片数
        ...
        down_proj = self.down_proj(self.act_fn(self.gate_proj(x, enabled_lora)) * self.up_proj(x, enabled_lora), enabled_lora)
    return down_proj
