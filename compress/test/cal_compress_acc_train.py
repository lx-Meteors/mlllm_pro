...
work_dir = "../mlllm-v2/compressLLM_long_text"
...
cl_generate_text = []
flag = True
if os.path.exists(work_dir + f'/pre-training_cl_generate_text.json'):
    with open(work_dir + f'/pre-training_cl_generate_text.json', 'r', encoding='utf-8') as f:
        for line in f:
            if line == "[\n" or line == "]\n" or line == "    {\n" or line == "    }\n":
                continue
            elif flag:
                line = line.replace("        \"input_text\": ", "", 1)
                input_text.append(line)
                flag = False
            else:
                flag = True
                line = line.replace("        \"cl_generate_text\": ", "", 1)
                cl_generate_text.append(line)
